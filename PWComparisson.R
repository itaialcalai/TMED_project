  ...
  tryCatch(
    {
      ...
      # scale the lof Fc after normilation
      logFC <- fit2$coefficients[, "groupT2D-normal"]
      logFC <- LinScale(logFC, min(logFC), max(logFC), -4, 4)
      fit2$coefficients[, "groupT2D-normal"] <- logFC
      ...
      top_up_text <- paste("\n\nSignificance cutoff by FDR < 0.05 (adj.P Value):\nTop up:", paste(top_up_genes, collapse = ", "))
      ...
      #add legend
      legend("topright", legend = "FDR Significant in color", xpd = TRUE)
      ...
      shape <- ifelse(plot_genes_subset$adj.P.Val < 0.05, 24, 16)
      ...
      plotMD(fit2, column=ct, status=dT[,ct], legend=F, pch=20, cex=1, main="MD plot (log fold change vs mean log expression)")
      abline(h=0)
      # add legend
      legend("topright", legend = "FDR Significant in color", xpd = TRUE)
